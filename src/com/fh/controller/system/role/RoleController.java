...
public class RoleController extends BaseController {
	...
	public ModelAndView listAllOrg(Model model,String ROLE_ID)throws Exception{
		ModelAndView mv = this.getModelAndView();
		try{
			Page page=new Page();
			PageData pd =new PageData();
			//pd.put("PARENT_ID", "1");					//上级ID
			page.setPd(pd);
			List<Department> depList = departmentService.listAllDepartment("0");	//列出Dictionaries列表
			JSONArray arr = JSONArray.fromObject(depList);
			String json = arr.toString();
			...
			model.addAttribute("zTreeNodes", json);
			mv.addObject("ROLE_ID",ROLE_ID);
			mv.setViewName("system/role/depqx");
		} catch(Exception e){
			logger.error(e.toString(), e);
		}
		return mv;
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/saveDepqx")
	public void saveDepqx(@RequestParam String ROLE_ID,@RequestParam String depIds,PrintWriter out)throws Exception{
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){} //校验权限
		logBefore(logger, Jurisdiction.getUsername()+"修改组织机构权限");
		PageData pd = new PageData();
		try{
			if(null != depIds && !"".equals(depIds.trim())){
				BigInteger depRights = RightsHelper.sumRights(Tools.str2StrArray(depIds));//用组织机构ID做权处理
				Role role = roleService.getRoleById(ROLE_ID);	//通过id获取角色对象
				role.setDEPRIGHTS(depRights.toString());
				roleService.updateDepRoleRights(role);				//更新当前角色组织机构权限
				pd.put("depRights",depRights.toString());
			}else{
				Role role = new Role();
				role.setDEPRIGHTS("");
				role.setROLE_ID(ROLE_ID);
				roleService.updateDepRoleRights(role);				//更新当前角色组织机构权限(没有任何勾选)
				pd.put("depRights","");
			}
				pd.put("ROLE_ID", ROLE_ID);
				roleService.setAllDEPRights(pd);					//更新此角色所有子角色的组织机构权限
			out.write("success");
			out.close();
		} catch(Exception e){
			logger.error(e.toString(), e);
