...
public class LoginController extends BaseController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/show_toIndex")
	...
	public Object login()throws Exception{
		...
		pd = this.getPageData();
		String errInfo = "";
		String KEYDATA[] = pd.getString("KEYDATA").replaceAll("username", "").replaceAll("password", "").split("~_~");
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				if((Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code))|| code.equalsIgnoreCase("noCheck")){		//判断登录验证码
					...
					if(pd != null){
						...
						user.setBIANMA(pd.getString("BIANMA"));
						user.setDepartment_id(pd.getString("DEPARTMENT_ID"));
						user.setSex(pd.getString("sex"));
						user.setSTATUS(pd.getString("STATUS"));
						PageData newPd=new PageData();
						newPd.put("DEPARTMENT_ID", pd.getString("DEPARTMENT_ID"));
						newPd = departmentService.findById(newPd);	//根据ID读取
						user.setDepartment(newPd);
	 ...
	 * @return
	 */
	public List<Department> readDep(List<Department> depList,String roleDepRights){
		for(int i=0;i<depList.size();i++){
			depList.get(i).setHasDepartment((RightsHelper.testRights(roleDepRights, depList.get(i).getDEPARTMENT_ID())));
			if(depList.get(i).isHasDepartment()){		//判断是否有此菜单权限
				this.readDep(depList.get(i).getSubDepartment(), roleDepRights);//是：继续排查其子菜单
			}else{
				depList.remove(i);
				i--;
			}
